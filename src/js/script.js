 ...
 */
window.interactedWithWebsite = false;
...
window.addEventListener("error", (err) => {
    const url = `https://github.com/kvbaxi/kuba.lol/issues/new?title=${encodeURIComponent(`Error in ${new URL(err.filename).pathname}`)}&body=${encodeURIComponent(`When I was <replace this section with what were you doing before> I got an error on ${err.filename} at line ${err.lineno} in column ${err.colno}:\n\n\`\`\`\n${err.message}\n\`\`\``)}`
    if (confirm([
        "There was an error when executing this operation:",
        "",
        `"${err.message}" at ${err.lineno}:${err.colno} (${new URL(err.filename).pathname})`,
        "",
        "Please report it ASAP to me at https://github.com/kvbaxi/kuba.lol/issues!",
 ...
 */
window.getRandomString = (length = 5) => {
...
window.isOnMobile = /Android|iPhone/i.test(navigator.userAgent)
 ...
 */
window.tryParseJSON = (jsonObjectIthink = "{}") => {
    let result;
    try{
 ...
 */
window.devtools_aknowledged = (localStorage.getItem("devtools_aknowledged") || "false") === "true";
    ...
    window.addEventListener('keydown', (ev) => {
        if(
            ...
            let aknowledged = confirm([
                ...
                "https://github.com/kvbaxi/kuba.lol/",
                "",
