// small check
window.SECRETS = {};
window.__smallCheck = () => Object.values(SECRETS).some(s => s.iconTriggered)
...
SECRETS.ONESHOT.iconTrigger = async (ev, iconElement) => {
    if(SECRETS.ONESHOT.acceptedOffer){
        ...
        let song = new Audio("/audio/oneshot_home.mp3")
        song.loop = true
        ...
        if (SECRETS.ONESHOT.brokenSunTimes === 0) await new Promise(ended => {
            ...
            let audio = new Audio("/audio/oneshot_shatter.mp3")
    ...
    nikoChar.src = '/img/oneshot_nikowalkL.gif'
    nikoChar.style.position = 'fixed';
    nikoChar.height = 80
    nikoChar.style.top = iconPos.y + nikoChar.height

    let sfx = new Audio('/audio/oneshot_step.mp3')
    sfx.loop = true;
    ...
    nikoChar.animate([
        ...
        nikoChar.src = '/img/oneshot_nikostandD.png'
        sfx.pause();
        setTimeout(() => {
            icon.animate([
                ...
                sfx.src = '/audio/oneshot_item_get.mp3'
                sfx.loop = false;
                icon.remove();
                nikoChar.src = '/img/oneshot_nikostandD_bulb.png'
                nikoChar.animate([
                    {filter: "brightness(5)"},
                ...
                sfx.addEventListener('ended', () => {
                    sfx.pause();
                    nikoChar.src = '/img/oneshot_nikowalkL_bulb.gif'
                    sfx.src = '/audio/oneshot_step.mp3'
...
SECRETS.OMORI.iconTrigger = (ev, iconElement) => {
    if(SECRETS.OMORI.iconSecret) {
        ...
        let sfx = new Audio('/audio/omori_gnihtemos.mp3')
        iconElement.querySelector('img.win-icon-icon').src = '/img/omori_something_d.gif'
    ...
    document.title = '\uFEFF'
    let sfx = new Audio("/audio/light_switch.mp3")
    ...
    setTimeout(() => {
        ...
        something.src = '/img/omori_something.png'
        ...
        sfx.src = "/audio/omori_SE_whispers.mp3"
        sfx.volume = 0.3
        sfx.loop = true;
        sfx.play();
        setTimeout(() => {
            ...
            setTimeout(() => {
                let sfx2 = new Audio('/audio/omori_SE_bs_shocking.mp3')
                ...
                sfx2.addEventListener('ended', () => {
                    ...
                    setTimeout(() => {
                        SECRETS.OMORI.globalSfx.src = '/audio/omori_SE_sucks_a_LOT.mp3'
                        SECRETS.OMORI.globalSfx.loop = true;
                        iconElement.querySelector('img.win-icon-icon').src = '/img/omori_something_m.png'
...
SECRETS.UNDERTALE.selectSFX = new Audio('/audio/undertale_snd_select.mp3')
SECRETS.UNDERTALE.moveSFX = new Audio('/audio/undertale_snd_squeak.mp3')
SECRETS.UNDERTALE.showSFX = new Audio('/audio/undertale_mus_cymbal.mp3')
...
SECRETS.UNDERTALE.iconTrigger = (ev, iconElement) => {
    ...
    closeIcon.src = '/img/undertale_close.png'
    Object.entries({
        height: '50px',
        position: 'fixed',
        right: 0,
