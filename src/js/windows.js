 ...
 * @returns {boolean}
 */
window.isUrl = (url) => /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()!@:%_\+.~#?&\/\/=]*)/.test(url)

window.isChristmasTime = () => new Date().getMonth() === 11 && new Date().getDate() >= 20 && new Date().getDate() <= 30

// Modifying wallpaper and lockscreen for config
if(isChristmasTime()) {
    WINDOWS.elem.style.backgroundImage = `url(/img/christmas-wallpaper-2.jpg)`
    document.querySelector('#win-login-bg').style.backgroundImage = `url(/img/christmas-wallpaper-1.jpg)`
} else {
...
WINDOWS.CLOCK.handler2137Secret = () => {
    const audio = new Audio('/audio/its_poping_time.mp3')
    audio.addEventListener('loadeddata', (ev) => {
        ...
        papamobil.src = '/img/its_poping_time.gif'
